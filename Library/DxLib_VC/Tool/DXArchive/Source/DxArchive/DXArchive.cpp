...
// ファイルの情報を得る
DARC_FILEHEAD *DXArchive::GetFileInfo( const char *FilePath )
{
	...
	// 同名のファイルを探す
	FileHeadSize = this->Head.Version >= 0x0002 ? sizeof( DARC_FILEHEAD ) : sizeof( DARC_FILEHEAD_VER1 ) ;
...
/*
	DARC_DIRECTORY *OldCurrent ;
	DARC_FILEHEAD *File ;
	char TPath[MAX_PATH], FPath[MAX_PATH], DPath[MAX_PATH] ;
	int i, len ;
		
	// 一文字もない場合は例外処理
	if( FilePath[0] == '\0' ) return ( DARC_FILEHEAD * )FileP ;
	...
	// ファイルパスの変更
	File = ( DARC_FILEHEAD * )( FileP + CurrentDirectory->FileHeadAddress ) ;
	for( 	i = 0 ;
...
int DXArchive::AddFileNameData( const char *FileName, u8 *FileNameTable )
{
...
// データを反転させる関数
void DXArchive::NotConv( void *Data , int Size )
{
	int DwordNum ;
	int ByteNum ;
...
void DXArchive::NotConvFileWrite( void *Data, int Size, FILE *fp )
{
	// データを反転する
	NotConv( Data, Size ) ;
...
void DXArchive::NotConvFileRead( void *Data, int Size, FILE *fp )
{
	// 読み込む
...
void DXArchive::KeyConvFileWrite( void *Data, int Size, FILE *fp, unsigned char *Key, int Position )
{
	int pos ;
...
void DXArchive::KeyConvFileRead( void *Data, int Size, FILE *fp, unsigned char *Key, int Position )
{
	int pos ;
...
int DXArchive::DirectoryEncode( char *DirectoryName, u8 *NameP, u8 *DirP, u8 *FileP, DARC_DIRECTORY *ParentDir, SIZESAVE *Size, int DataNumber, FILE *DestP, void *TempBuffer, bool Press, unsigned char *Key )
{
			...
			else
			{
				...
				if( (u64)FindData.nFileSizeLow + ( (u64)FindData.nFileSizeHigh << 32 ) != 0 )
				{
					...
					if( Press == true && (u64)FindData.nFileSizeLow + ( (u64)FindData.nFileSizeHigh << 32 ) < 10 * 1024 * 1024 )
					{
						...
						KeyConvFileWrite( DestBuf, WriteSize, DestP, Key ) ;
...
NOPRESS:					
						...
						while( WriteSize < FileSize )
						{
							...
							// ファイルの反転読み込み
							KeyConvFileRead( TempBuffer, MoveSize, SrcP, Key, ftell( DestP ) ) ;
...
int DXArchive::Encode( void *Src, unsigned int SrcSize, void *Dest )
{
	s32 dstsize ;
	...
	listbuf        = (LZ_LIST *)( sublistbuf + sizeof( u32 ) * 256 * sublistmaxnum ) ;
	...
	while( srcaddress < SrcSize )
	{
		...
		// リストを取得
		code = *((u16 *)sp) ;
		list = (LZ_LIST *)( listfirsttable + code * sizeof( u32 ) ) ;
		if( usesublistflagtable[code] == 1 )
		{
			list = (LZ_LIST *)( (u32 *)list->next + sp[2] ) ;
		}
		else
		{
			if( sublistnum < sublistmaxnum )
			{
				list->next = (LZ_LIST *)( sublistbuf + sizeof( u32 ) * 256 * sublistnum ) ;
				list       = (LZ_LIST *)( (u32 *)list->next + sp[2] ) ;
		...
		else
		{
			...
			if( srcaddress + maxconbo < SrcSize )
			{
				...
				for( j = 1 ; j < maxconbo && (u32)&sp2[2] - (u32)srcp < SrcSize ; j ++, sp2 ++ )
				{
					...
					if( usesublistflagtable[code] == 1 )
					{
						list = (LZ_LIST *)( (u32 *)list->next + sp2[2] ) ;
					}
					else
					{
						if( sublistnum < sublistmaxnum )
...
int DXArchive::EncodeArchive( char *OutputFileName, char **FileOrDirectoryPath, int FileNum, bool Press, const char *KeyString )
{
	...
	// アーカイブのヘッダを出力する
	{
		...
		KeyConvFileWrite( &Head, sizeof( DARC_HEAD ), DestFp, Key ) ;
	}
	
	// 各バッファを確保する
	if( ( NameP = ( u8 * )malloc( DXA_BUFFERSIZE ) ) == NULL ) return -1 ;
	memset( NameP, 0, DXA_BUFFERSIZE ) ;

	if( ( FileP = ( u8 * )malloc( DXA_BUFFERSIZE ) ) == NULL ) return -1 ;
	memset( FileP, 0, DXA_BUFFERSIZE ) ;

	if( ( DirP = ( u8 * )malloc( DXA_BUFFERSIZE ) ) == NULL ) return -1 ;
		...
		else
		{
			...
			if( (u64)FindData.nFileSizeLow + ( (u64)FindData.nFileSizeHigh << 32 ) != 0 )
			{
				...
				if( Press == true && (u64)FindData.nFileSizeLow + ( (u64)FindData.nFileSizeHigh << 32 ) < 10 * 1024 * 1024 )
				{
					...
					// 圧縮データを反転して書き出す
					WriteSize = ( DestSize + 3 ) / 4 * 4 ;
					KeyConvFileWrite( DestBuf, WriteSize, DestFp, Key ) ;
...
NOPRESS:					
					...
					while( WriteSize < FileSize )
					{
						...
						// ファイルの反転読み込み
						KeyConvFileRead( TempBuffer, MoveSize, SrcP, Key, ftell( DestFp ) ) ;
	...
	// バッファに溜め込んだ各種ヘッダデータを出力する
	{
		...
		KeyConvFileWrite( NameP, SizeSave.NameSize, DestFp, Key ) ;
		KeyConvFileWrite( FileP, SizeSave.FileSize, DestFp, Key ) ;
		KeyConvFileWrite( DirP, SizeSave.DirectorySize, DestFp, Key ) ;
	}
...
int DXArchive::DecodeArchive( char *ArchiveName, char *OutputPath, const char *KeyString )
{
	...
	// ヘッダを解析する
	{
		KeyConvFileRead( &Head, sizeof( DARC_HEAD ), ArcP, Key ) ;
		...
		// バージョン検査
		if( Head.Version > DXA_VER ) goto ERR ;
...
int	DXArchive::OpenArchiveMem( void *ArchiveImage, int ArchiveSize, const char *KeyString )
{
	u8 *datp ;
	...
	{
		// バージョン２以前か調べる
		memset( this->Key, 0xffffffff, DXA_KEYSTR_LENGTH ) ;
	...
	// ヘッダを解析する
	{
		...
		// バージョン検査
		if( this->Head.Version > DXA_VER ) goto ERR ;
...
int DXArchive::OpenArchiveFile( const char *ArchivePath, const char *KeyString )
{
	...
	// ヘッダを解析する
	{
		KeyConvFileRead( &this->Head, sizeof( DARC_HEAD ), this->fp, this->Key ) ;
		...
		if( this->Head.Head != DXA_HEAD )
		{
			...
			KeyConvFileRead( &Head, sizeof( DARC_HEAD ), this->fp, this->Key ) ;
...
int DXArchive::LoadFileToMem( const char *FilePath, void *Buffer, unsigned int BufferLength )
{
	DARC_FILEHEAD *FileH ;
...
int DXArchive::GetFileSize( const char *FilePath )
{
	// ファイルサイズを返す
...
int DXArchive::GetFileInfo( const char *FilePath, int *PositionP, int *SizeP )
{
	DARC_FILEHEAD *FileH ;
...
DXArchiveFile::DXArchiveFile( DARC_FILEHEAD *FileHead, DXArchive *Archive )
{
	...
	if( this->Archive->GetHeader()->Version >= 0x0002 && FileHead->PressDataSize != 0xffffffff )
	{
		...
		DXArchive::KeyConvFileRead( temp, FileHead->PressDataSize, this->Archive->GetFilePointer(), this->Archive->GetKey() ) ;
...
// ファイルの内容を読み込む
int DXArchiveFile::Read( void *Buffer, int ReadLength )
{
	int ReadSize ;
	...
	if( this->DataBuffer == NULL )
	{
		DXArchive::KeyConvFileRead( Buffer, ReadSize, this->Archive->GetFilePointer(), this->Archive->GetKey() ) ;
	}
	else
	{
...
// ファイルポインタを変更する
int DXArchiveFile::Seek( int SeekPoint, int SeekMode )
{
	// シークタイプによって処理を分岐
	switch( SeekMode )
	{
...
// 現在のファイルポインタを得る
int DXArchiveFile::Tell( void )
{
	return this->FilePoint ;
}

// ファイルの終端に来ているか、のフラグを得る
int DXArchiveFile::Eof( void )
{
	return this->EOFFlag ;
}

// ファイルのサイズを取得する
int DXArchiveFile::Size( void )
{
