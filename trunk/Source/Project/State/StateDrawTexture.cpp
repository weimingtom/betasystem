...
{
	DrawGraph(0,0, Game::ImageLoader::GetInstance()->ImageHandleOf( static_cast<Game::ImageType>(mIndex) ), FALSE );
    ...
    mIndex = Clamp( 0, mIndex, Game::ImageType_Num - 1 );

	DrawFormatString( 0+1 , 40+1 , ColorOf(0,0,0) , "%s", Game::ImageLoader::NameOf( static_cast<Game::ImageType>(mIndex) ) );
    DrawFormatString( 0 , 40 , ColorOf(230,230,230) , "%s", Game::ImageLoader::NameOf( static_cast<Game::ImageType>(mIndex) ) );
}
