...
void Snow::Init()
{
	/* 初期座標設定 */
	this->SetPoint(
		rand() % (SCREEN_W - IMGSIZE_SNOW),
		-IMGSIZE_SNOW);

	/* 移動量設定 */
	this->SetMove(
		rand() % 100 * 0.0001 - 0.005,
		rand() % 100 * 0.01   + 0.7);
...
bool Snow::Move()
{
	...
	/* X軸移動 */
	this->p.x += this->mv.x;
	if(this->p.x < 0 - IMGSIZE_SNOW || SCREEN_W < this->p.x)
	{
		ret = false;
	}
	...
	/* Y軸移動 */
	this->p.y += this->mv.y;
	if(this->p.y > SCREEN_H + IMGSIZE_SNOW)
	{
		this->p.y = SCREEN_H + IMGSIZE_SNOW;
		/* 表示終了 */
		ret = false;
	}
